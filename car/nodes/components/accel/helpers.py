...
class I2C:
    ...
    def write_byte(self, register, byte):
        ...
        while True:
            try:
                self.bus.write_byte_data(self.address, register, byte)
                logger.debug('I2C: Wrote 0x%02X to register 0x%02X', byte,
                             register)
                break
            except IOError, err:
                logger.exception(
                    'Error %d, %s accessing 0x%02X: Check your I2C address',
                    err.errno, err.strerror, self.address)
                time.sleep(0.001)
    ...
    def write_block(self, register, block):
        ...
        while True:
            try:
                self.bus.write_i2c_block_data(self.address, register, block)
                break
            except IOError, err:
                logger.exception(
                    'Error %d, %s accessing 0x%02X: Check your I2C address',
        ...
        while True:
            try:
                ...
                logger.debug(
                    ...
                    self.address, result & 0xFF, register)
                return result
            except IOError, err:
                logger.exception(
                    'Error %d, %s accessing 0x%02X: Check your I2C address',
    ...
    def read_block(self, register, length):
        ...
        while True:
            try:
                result = self.bus.read_i2c_block_data(self.address, register,
                                                      length)
                logger.debug('I2C: Device 0x%02X from reg 0x%02X',
                             self.address,
                             register)
                return result
            except IOError, err:
...
class MPU6050:
    ...
    # Registers/etc.
    __MPU6050_RA_XG_OFFS_TC = 0x00
    # [7] PWR_MODE, [6:1] XG_OFFS_TC, [0] OTP_BNK_VLD
    __MPU6050_RA_YG_OFFS_TC = 0x01
    # [7] PWR_MODE, [6:1] YG_OFFS_TC, [0] OTP_BNK_VLD
    __MPU6050_RA_ZG_OFFS_TC = 0x02       # [7] PWR_MODE, [6:1] ZG_OFFS_TC,
    # [0] OTP_BNK_VLD
    ...
    def __init__(self, address=0x68):
        ...
        self.i2c.write_byte(self.__MPU6050_RA_PWR_MGMT_1, 0x80)
        time.sleep(2)

        # Sets sample rate to 1000/1+4 = 200Hz
        logger.debug('Sample rate 200Hz')
        self.i2c.write_byte(self.__MPU6050_RA_SMPLRT_DIV, 0x04)
        time.sleep(0.005)

        # Sets clock source to gyro reference w/ PLL
        logger.debug('Clock gyro PLL')
        self.i2c.write_byte(self.__MPU6050_RA_PWR_MGMT_1, 0x02)
        time.sleep(0.005)

        # Controls frequency of wakeups in accel low power mode plus the
        # sensor standby modes
        logger.debug('Disable low-power')
        self.i2c.write_byte(self.__MPU6050_RA_PWR_MGMT_2, 0x00)
        time.sleep(0.005)

        # Disable gyro self tests, scale of +/- 500 degrees/s
        logger.debug('Gyro +/-500 degrees/s')
        self.i2c.write_byte(self.__MPU6050_RA_GYRO_CONFIG, 0x08)
        time.sleep(0.005)

        # Disable accel self tests, scale of +/-2g
        logger.debug('Accel +/- 2g')
        self.i2c.write_byte(self.__MPU6050_RA_ACCEL_CONFIG, 0x00)
        time.sleep(0.005)

        # Setup INT pin to latch and AUX I2C pass through
        logger.debug('Enable interrupt')
        self.i2c.write_byte(self.__MPU6050_RA_INT_PIN_CFG, 0x20)
        time.sleep(0.005)

        # Enable data ready interrupt
        logger.debug('Interrupt data ready')
        self.i2c.write_byte(self.__MPU6050_RA_INT_ENABLE, 0x01)
        time.sleep(0.005)

        # Disable FSync, 5Hz DLPF => 1kHz sample frequency used above
        # divided by the
        # sample divide factor.
        logger.debug('5Hz DLPF')
        # 0x05 => 10Hz DLPF
        self.i2c.write_byte(self.__MPU6050_RA_CONFIG, 0x06)
        time.sleep(0.005)

        logger.debug('Gumph hereafter...')

        # Freefall threshold of |0mg|
        self.i2c.write_byte(self.__MPU6050_RA_FF_THR, 0x00)
        time.sleep(0.005)

        # Freefall duration limit of 0
        self.i2c.write_byte(self.__MPU6050_RA_FF_DUR, 0x00)
        time.sleep(0.005)

        # Motion threshold of 0mg
        self.i2c.write_byte(self.__MPU6050_RA_MOT_THR, 0x00)
        time.sleep(0.005)

        # Motion duration of 0s
        self.i2c.write_byte(self.__MPU6050_RA_MOT_DUR, 0x00)
        time.sleep(0.005)

        # Zero motion threshold
        self.i2c.write_byte(self.__MPU6050_RA_ZRMOT_THR, 0x00)
        time.sleep(0.005)

        # Zero motion duration threshold
        self.i2c.write_byte(self.__MPU6050_RA_ZRMOT_DUR, 0x00)
        time.sleep(0.005)

        # Disable sensor output to FIFO buffer
        self.i2c.write_byte(self.__MPU6050_RA_FIFO_EN, 0x00)
        time.sleep(0.005)

        # AUX I2C setup
        # Sets AUX I2C to single master control, plus other config
        self.i2c.write_byte(self.__MPU6050_RA_I2C_MST_CTRL, 0x00)
        time.sleep(0.005)

        # Setup AUX I2C slaves
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV0_ADDR, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV0_REG, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV0_CTRL, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV1_ADDR, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV1_REG, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV1_CTRL, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV2_ADDR, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV2_REG, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV2_CTRL, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV3_ADDR, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV3_REG, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV3_CTRL, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV4_ADDR, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV4_REG, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV4_DO, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV4_CTRL, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV4_DI, 0x00)

        # Slave out, dont care
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV0_DO, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV1_DO, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV2_DO, 0x00)
        self.i2c.write_byte(self.__MPU6050_RA_I2C_SLV3_DO, 0x00)

        # More slave config
        self.i2c.write_byte(self.__MPU6050_RA_I2C_MST_DELAY_CTRL, 0x00)
        time.sleep(0.005)

        # Reset sensor signal paths
        self.i2c.write_byte(self.__MPU6050_RA_SIGNAL_PATH_RESET, 0x00)
        time.sleep(0.005)

        # Motion detection control
        self.i2c.write_byte(self.__MPU6050_RA_MOT_DETECT_CTRL, 0x00)
        time.sleep(0.005)

        # Disables FIFO, AUX I2C, FIFO and I2C reset bits to 0
        self.i2c.write_byte(self.__MPU6050_RA_USER_CTRL, 0x00)
        time.sleep(0.005)

        # Data transfer to and from the FIFO buffer
        self.i2c.write_byte(self.__MPU6050_RA_FIFO_R_W, 0x00)
        time.sleep(0.005)

    def read_sensors_raw(self):
        ...
        self.i2c.read_unsigned_byte(self.__MPU6050_RA_INT_STATUS)

        # Hard loop on the data ready interrupt until it gets set high
        while not (self.i2c.read_unsigned_byte(
                self.__MPU6050_RA_INT_STATUS) == 0x01):
            time.sleep(0.001)
            continue

        # Disable the interrupt while we read the data
        self.i2c.write_byte(self.__MPU6050_RA_INT_ENABLE, 0x00)

        # For speed of reading, read all the sensors and parse to USHORTs after
        sensor_data = self.i2c.read_block(self.__MPU6050_RA_ACCEL_XOUT_H, 14)

        for index in range(0, 14, 2):
            if sensor_data[index] > 127:
                sensor_data[index] -= 256
            self.result_array[int(index / 2)] = (
                (sensor_data[index] << 8) + sensor_data[index + 1]
            )
    ...
    def read_sensors(self):
        ...
        [ax, ay, az, temperature, gx, gy, gz] = self.read_sensors_raw()
        fax = float(
            ax * self.__CALIBRATION_ITERATIONS - self.ax_offset
        ) * 4.0 / float(65536 * self.__CALIBRATION_ITERATIONS)
        fay = float(
            ay * self.__CALIBRATION_ITERATIONS - self.ay_offset
        ) * 4.0 / float(65536 * self.__CALIBRATION_ITERATIONS)
        faz = float(
            az * self.__CALIBRATION_ITERATIONS - self.az_offset
        ) * 4.0 / float(65536 * self.__CALIBRATION_ITERATIONS)

        fgx = float(
            gx * self.__CALIBRATION_ITERATIONS - self.gx_offset
        ) * 1000.0 / float(65536 * self.__CALIBRATION_ITERATIONS)
        fgy = float(
            gy * self.__CALIBRATION_ITERATIONS - self.gy_offset
        ) * 1000.0 / float(65536 * self.__CALIBRATION_ITERATIONS)
        fgz = float(
            gz * self.__CALIBRATION_ITERATIONS - self.gz_offset
        ) * 1000.0 / float(65536 * self.__CALIBRATION_ITERATIONS)

        return fax, fay, faz, fgx, fgy, fgz

    #noinspection PyUnusedLocal
    def update_offsets(self, file_name):
        """
        ...
        try:
            ...
            logger.critical(
                'Could not open offset config file: %s for writing',
                file_name)
            cfg_rc = False

        return cfg_rc

    def read_offsets(self, file_name):
        """
        ...
        try:
            ...
            logger.critical(
                'Could not open offset config file: %s for reading',
                file_name)
            cfg_rc = False
    ...
    @staticmethod
    def get_euler_angles(fax, fay, faz):
        """
        ...
        theta = math.atan2(fax,
                           math.pow(math.pow(fay, 2) + math.pow(faz, 2), 0.5))
        psi = math.atan2(fay,
                         math.pow(math.pow(fax, 2) + math.pow(faz, 2), 0.5))
        phi = math.atan2(math.pow(math.pow(fax, 2) + math.pow(fay, 2), 0.5),
                         faz)
    ...
    def read_temperature(self):
        ...
        @return: float
        """
        temp = self.i2c.read_signed_word(self.__MPU6050_RA_TEMP_OUT_H)
...
class PID:
    ...
    def compute(self, reading, target):
        """
        Compute PID regulator values.
        ...
        # The proportional term takes the distance between current input and
        # target
        # and uses this proportially (based on Kp) to control the blade speeds
        p_error = error

        # The integral term sums the errors across many compute calls to
        # allow for
        ...
        # The differential term accounts for the fact that as error
        # approaches 0,
        # the output needs to be reduced proportionally to ensure factors
        # such as
        # momentum do not cause overshoot.
        d_error = (error - self.last_error) / dt

        # The overall output is the sum of the (P)roportional, (I)ntegral
        # and (D)iffertial terms
        p_output = self.p_gain * p_error
        i_output = self.i_gain * i_error
        d_output = self.d_gain * d_error

        # Store off last input (for the next differential calulation) and
        # time for calculating the integral value
        self.last_error = error
        self.last_time = now

        # Return the output, which has been tuned to be the increment /
        # decrement in blade PWM
