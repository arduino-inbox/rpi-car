    ...
    def readList(self, reg, length):
        ...
        while True:
            try:
                ...
                logger.debug('I2C: Device 0x%02X from reg 0x%02X', self.address,
                             reg)
                return result
            except IOError, err:
                logger.exception(
...
class MPU6050:
    ...
    def readSensors(self):
        ...
        fax = float(
            ax * self.__CALIBRATION_ITERATIONS - self.ax_offset) * 4.0 / float(
            65536 * self.__CALIBRATION_ITERATIONS)
        fay = float(
            ay * self.__CALIBRATION_ITERATIONS - self.ay_offset) * 4.0 / float(
            65536 * self.__CALIBRATION_ITERATIONS)
        faz = float(
            az * self.__CALIBRATION_ITERATIONS - self.az_offset) * 4.0 / float(
            65536 * self.__CALIBRATION_ITERATIONS)
        fgx = float(
            gx * self.__CALIBRATION_ITERATIONS - self.gx_offset) * 1000.0 / float(
            65536 * self.__CALIBRATION_ITERATIONS)
        fgy = float(
            gy * self.__CALIBRATION_ITERATIONS - self.gy_offset) * 1000.0 / float(
            65536 * self.__CALIBRATION_ITERATIONS)
        fgz = float(
            gz * self.__CALIBRATION_ITERATIONS - self.gz_offset) * 1000.0 / float(
            65536 * self.__CALIBRATION_ITERATIONS)
    ...
    def updateOffsets(self, file_name):
        ...
        try:
            ...
            logger.critical('Could not open offset config file: %s for writing',
                            file_name)
            cfg_rc = False
    ...
    def readOffsets(self, file_name):
        ...
        try:
            ...
            logger.critical('Could not open offset config file: %s for reading',
                            file_name)
            cfg_rc = False
